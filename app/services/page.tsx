  ...
  return (
    <>
      <Section rule={false} className="pt-[clamp(130px,15vh,190px)]">
        <SectionHead title={<>Five services<br />that fit together.</>}>
          <p className="max-w-[60ch] text-steel">
            Take one on its own, or run the whole sequence from a blank page to a website that earns its own traffic.
            Every engagement is scoped and priced in writing before it starts.
          </p>
        </SectionHead>
      </Section>
        ...
        <div className="border-t border-hair">
          {services.map((service) => (
            <article
              key={service.slug}
              id={service.slug}
              className="grid scroll-mt-28 gap-[clamp(20px,4vw,56px)] border-b border-hairsoft py-[clamp(28px,3.4vw,44px)] lg:grid-cols-[minmax(0,0.5fr)_minmax(0,1fr)_minmax(0,0.72fr)]"
            >
              <h2 className="display text-[clamp(1.2rem,1.7vw,1.45rem)] leading-tight">{service.title}</h2>
              <p className="text-[0.98rem] text-steel">{service.summary}</p>
              <ul>
                {service.deliverables.map((d) => (
                  <li key={d} className="border-b border-hairsoft py-1.5 text-[0.88rem] text-steel last:border-b-0">{d}</li>
                ))}
              </ul>
            </article>
          ))}
          ...
          {pricing.map((plan, i) => (
            <Reveal key={plan.title} delay={i * 0.06}>
              <article className="flex h-full flex-col gap-3 rounded border border-hair p-[clamp(24px,2.6vw,34px)]">
                <h3 className="text-[1.05rem] font-medium text-bright">{plan.title}</h3>
                <p className="font-[family-name:var(--font-bodoni)] text-[1.65rem] leading-none text-champagne">{plan.figure}</p>
                <p className="text-[0.92rem] text-steel">{plan.body}</p>
                <p className="mt-auto border-t border-hairsoft pt-4 text-[0.84rem] text-silver">{plan.note}</p>
              </article>
            </Reveal>
          ))}
        </div>
      ...
      <Section>
        ...
        <ul className="mt-[clamp(38px,5vw,64px)] grid border-t border-hair lg:grid-cols-3">
          {needs.map((n, i) => (
            <Reveal as="li" key={n.h} delay={i * 0.06} className="h-full border-b border-hairsoft py-7 pr-7 lg:border-b-0 lg:border-r lg:last:border-r-0">
                <h3 className="text-[1rem] font-medium text-bright">{n.h}</h3>
                <p className="mt-2 text-[0.9rem] text-steel">{n.p}</p>
              </Reveal>
          ))}
        </ul>
      </Section>
