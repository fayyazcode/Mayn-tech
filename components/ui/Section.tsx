...
export function Section({
  ...
  return (
    <section id={id} className={`py-[clamp(68px,8.5vw,132px)] ${rule ? "border-t border-line" : ""} ${className}`}>
...
export function SectionHead({ title, children }: { title: ReactNode; children?: ReactNode }) {
  return (
    <div className="grid items-start gap-[clamp(28px,5vw,84px)] lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
      <h2 className="display min-w-0 text-[clamp(1.85rem,3.5vw,2.85rem)] break-words">{title}</h2>
